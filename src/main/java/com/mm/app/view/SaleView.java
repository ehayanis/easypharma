...
import com.mm.app.utilities.Constants;
...
public class SaleView extends JFrame {
	...
	private void initComponents() {
		...
		jTable1.setModel(new DefaultTableModel(new Object[][] { { null, null, null, null, null, null },
				...
				// {null, null, null, null, null, null},
				// {null, null, null, null, null, null}
		}, new String[] { "Désignation", "Liste Remb.", "PU TTC", "Qté", "Remise", "Total" }));
		...
		if (result != null && result.size() > 0) {
			for (Product p : result) {
				data.put(p.getDesignation(), p.getId().toString());
				data.put(p.getReference(), p.getId().toString());
			}
		}
			...
			public void tableChanged(TableModelEvent evt) {
				if (evt.getType() == TableModelEvent.UPDATE) {
					...
					if (Utilities.isNotEmpty(jTable1.getValueAt(row, Constants.DESIGNATIONCMLN)) && (column == Constants.QUANTITYCLMN || column == Constants.DISCOUNTCLMN)) {
						...
						// unitPrice
						Double unitPrice = 0D;
						if (Utilities.isNotEmpty(jTable1.getValueAt(row, Constants.UNITPRICECMLN))) {
							unitPrice = new Double(jTable1.getValueAt(row, Constants.UNITPRICECMLN).toString().replace(",", "."));
						}

						// quantity
						Double quantity = 0D;
						if (Utilities.isNotEmpty(jTable1.getValueAt(row, Constants.QUANTITYCLMN))) {
							quantity = new Double(jTable1.getValueAt(row, Constants.QUANTITYCLMN).toString().replace(",", "."));
						}

						// discount
						Double discount = 0D;
						if (Utilities.isNotEmpty(jTable1.getValueAt(row, Constants.DISCOUNTCLMN))) {
							discount = new Double(jTable1.getValueAt(row, Constants.DISCOUNTCLMN).toString().replace(",", "."));
						}

						// calculate the rowSum
						Double rowSum = unitPrice * quantity;
						rowSum = rowSum * (1 - discount / 100);
						jTable1.setValueAt(Constants.DECIMALFORMAT.format(rowSum), row, Constants.ROWSUMCLMN);
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyChar() == KeyEvent.VK_DELETE) {
					...
					if (products.get(row) != null) {
						...
						// Remise à zéro de la colone dsésignation, les lines
						// précédentes
		...
		// jMenuItem1.setAccelerator(KeyStroke.getKeyStroke(KeyVK_N,
		// InputCTRL_MASK));
		...
		// jMenuItem3.setAccelerator(KeyStroke.getKeyStroke(KeyVK_C,
		// InputCTRL_MASK));
		...
		layout.setVerticalGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(layout.createSequentialGroup()
				...
				.addPreferredGap(LayoutStyle.ComponentPlacement.RELATED).addComponent(jPanel1, GroupLayout.PREFERRED_SIZE, 219, GroupLayout.PREFERRED_SIZE)
				...
				// GroupLayout.PREFERRED_SIZE, 580,
				// GroupLayout.PREFERRED_SIZE)
				.addPreferredGap(LayoutStyle.ComponentPlacement.RELATED)
				.addComponent(footerPanel, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE).addContainerGap()));
		...
		Action f9Action = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				if ("".equals(footerPanel.getTotalValue().getText())) {
					...
					// PosologieFrame posologieFrame = new PosologieFrame(em,
					// vente);
					vente.setProduits(products.values());
	...
	private void updateTotal() {
		...
		for (int i = 0; i < rows; i++) {
			if (Utilities.isNotEmpty(jTable1.getValueAt(i, Constants.ROWSUMCLMN))) {
				Double rowTotal = new Double(jTable1.getValueAt(i, Constants.ROWSUMCLMN).toString().replace(",", "."));
				total += rowTotal;
			}
		}
		footerPanel.getTotalValue().setText(Constants.DECIMALFORMAT.format(total));
	}
