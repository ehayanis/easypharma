        ...
        firstName.getEditor().getEditorComponent().addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                ...
                if (e.getKeyChar() == KeyEvent.VK_ENTER) {
                    ...
                    JTextField typedTextField = (JTextField) firstName.getEditor().getEditorComponent();
                    if (typedTextField.getText() != null && !comboBoxModel.contains(typedTextField.getText()) && !typedTextField.getText().equals(lastTypedText)) {
                        ...
                        List<Client> result = clientService.findClientsByCriteria(lastTypedText.toLowerCase());
                        data.clear();
                        if(result != null && result.size() > 0){
                            for(Client c : result){
                                data.put(c.getFirstName() + " " + c.getLastName(), c.getReference());
                        ...
                        firstName.setModel(new DefaultComboBoxModel(comboBoxModel.toArray()));
